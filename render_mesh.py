...
from DataUtil.ObjIO import load_obj_data, load_ply_data, save_obj_data, save_obj_data_binary
import glob
import time
import argparse
...
class render_mesh(object):
    ...
    def start_rendering(self, accuRotDegree, info, dataPrefix, meshPath):
        ...
        # render rgbImage
        self.rgbImage = self.rn.r #self.renderRGB() # (h,w,3), values in (0,1)
    ...
    def voxelize(self,meshPath,info):
        ...
        if info == "FRONT":
            ...
            meshPathNew = meshPath.replace(".obj","_normalized.obj")
...
def parse_args():
    ...
    parser.add_argument('--meshDirSearch', type=str, default="/mnt/data/Project/Geo-PIFu-master/data")
    parser.add_argument('--bgDirSearch', type=str, default="/mnt/data/Project/Geo-PIFu-master/data")
    parser.add_argument('--saveDir', type=str, default="/mnt/data/Project/Geo-PIFu-master/data/humanRender")
...
def read_mesh_paths_buff(meshDirSearch):
    """
    read a list of meshe paths, N (6795)
    """

    print("reading mesh paths...")
    personClothesDirs = glob.glob("%s/THuman2.0/0*" % (meshDirSearch)) # 202
    meshPaths = [] # 385
    for dirEach in personClothesDirs:
        meshPathsTmp = glob.glob("%s/*.obj" % (dirEach))
        meshPaths.extend(meshPathsTmp)
    meshNum = len(meshPaths) # 385
    print(meshNum)
...
def loaded_config_consistency_check(dataPrefix,meshPath):
    ...
    loadedConfig["meshPath"] = loadedConfig["meshPath"].replace("/trainman-mount/trainman-storage-d5c0a121-bb5d-4afb-8020-c53f096d2a5c","/mnt/data/Project/Geo-PIFu-master")
    loadedConfig["args"]["bgDirSearch"] = loadedConfig["args"]["bgDirSearch"].replace(
        "/trainman-mount/trainman-storage-d5c0a121-bb5d-4afb-8020-c53f096d2a5c", "/mnt/data/Project/Geo-PIFu-master")
    loadedConfig["args"]["saveDir"] = loadedConfig["args"]["saveDir"].replace(
        "/trainman-mount/trainman-storage-d5c0a121-bb5d-4afb-8020-c53f096d2a5c", "/mnt/data/Project/Geo-PIFu-master")
    loadedConfig["args"]["meshDirSearch"] = loadedConfig["args"]["meshDirSearch"].replace(
        "/trainman-mount/trainman-storage-d5c0a121-bb5d-4afb-8020-c53f096d2a5c", "/mnt/data/Project/Geo-PIFu-master")
    loadedConfig["bgImgPath"] = loadedConfig["bgImgPath"].replace(
        "/trainman-mount/trainman-storage-d5c0a121-bb5d-4afb-8020-c53f096d2a5c", "/mnt/data/Project/Geo-PIFu-master")
    ...
    return loadedConfig["meshPath"], loadedConfig
...
def main(args):
    ...
    # read a list of meshe paths, N (6795) / buff(385)
    meshPaths, meshNum = read_mesh_paths_buff(meshDirSearch=args.meshDirSearch)
    # print(meshPaths[0])
    ...
    for meshIdx in range(meshNum):
        ...
        for nameIdx in range(args.eachMeshWithBgNum):
            ...
            if args.useConfig:
                meshPaths[meshIdx], loadedConfig = loaded_config_consistency_check(dataPrefix=dataPrefix,meshPath=meshPaths[meshIdx])
                ...
                render.render_colored_mesh_with_background(meshPath=meshPaths[meshIdx],bgImgPath=None,quickDemo=args.quickDemo, dataPrefix=dataPrefix,eachMeshWithBgNum=args.eachMeshWithBgNum)
