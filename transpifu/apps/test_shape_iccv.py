...
from lib.model.swin_Transformer import CONFIGS as CONFIGS_ViT_seg
from lib.model.TransPIFuNet import TransPIFuNet
import time
...
def compute_split_range(testing_inds, args):
    ...
    for eachTestIdx in testing_inds[splitRange[0]:splitRange[1]]:
        ...
        normalPath = "%s/normalRGB/%06d.jpg"%(args.datasetDir,eachTestIdx)
        depthPath = "%s/depthRGB/%06d.jpg" % (args.datasetDir, eachTestIdx)
        ...
        assert (os.path.exists(normalPath))  # normal image
        assert (os.path.exists(depthPath))  # depth image
        
        # save idx
        testIdxList.append([configPath,maskPath,rgbImagePath,normalPath,depthPath])
...
def use_provided_idx(args):
    ...
    for eachTestIdx in args.give_idx:
        ...
        # save idx
        testIdxList.append([configPath,maskPath,rgbImagePath,normalPath])
...
class Evaluator:
    ...
    def __init__(self, opt, projection_mode='orthogonal'):
        ...
        if opt.name == "PIFu_transformer":
            config_vit = CONFIGS_ViT_seg[opt.vit_name]
            config_vit.n_skip = opt.n_skip
            if opt.vit_name.find('R50') != -1:
                config_vit.patches.grid = (
                int(opt.img_size / opt.vit_patches_size), int(opt.img_size / opt.vit_patches_size))

            # {create, deploy} networks to the specified GPU
            netG = TransPIFuNet(opt, config_vit, projection_mode, img_size=opt.img_size).to(device=cuda)
        ...
        if opt.load_netG_checkpoint_path:
            ...
            if not self.opt.load_from_multi_GPU_shape: netG.load_state_dict(torch.load(self.opt.load_netG_checkpoint_path, map_location=cuda),strict=False)
    ...
    def load_image(self, depth_path, normal_path, image_path, mask_path, configPath, idx, args):
        ...
        if True:
            ...
            # convert to (3, 512, 512) tensors, RGB, float, -1 ~ 1. note that bg is 0 not -1.
            normal_padded = self.to_tensor(normal_padded) # (3, 512, 512), float -1 ~ 1
            normal_padded = mask_data_padded.expand_as(normal_padded) * normal_padded

            dict_to_return["normal"] = normal_padded.unsqueeze(0) # VCHW, (1,3,512,512), float -1 ~ 1, bg are all ZEROS, not -1.

        # ----- load depth -----
        if True:
            # read data BGR -> RGB, np.uint8
            depth = cv2.imread(depth_path)[:, :, ::-1]  # (1536, 1024, 3), np.uint8, {0,...,255}
            depth_padded = np.zeros((max(depth.shape), max(depth.shape), 3), np.uint8)  # (1536, 1536, 3)
            depth_padded[:,depth_padded.shape[0] // 2 - min(depth.shape[:2]) // 2:depth_padded.shape[0] // 2 + min(depth.shape[:2]) // 2, :] = depth  # (1536, 1536, 3)

            # resize to (512, 512, 3), np.uint8
            depth_padded = cv2.resize(depth_padded, (self.opt.loadSize, self.opt.loadSize))
            depth_padded = Image.fromarray(depth_padded)

            # convert to (3, 512, 512) tensors, RGB, float, -1 ~ 1. note that bg is 0 not -1.
            depth_padded = self.to_tensor(depth_padded)  # (3, 512, 512), float -1 ~ 1
            depth_padded = mask_data_padded.expand_as(depth_padded) * depth_padded

            dict_to_return["depth"] = depth_padded.unsqueeze(0)  # VCHW, (1,3,512,512), float -1 ~ 1, bg are all ZEROS, not -1.
...
def main(args):
    ...
    for pathList in testIdxList:
        ...
        if os.path.exists(save_path_obj):
            continue;
        """
        load data
            'name': meshName,
        ...
        """
        data = evaluator.load_image(depth_path=pathList[4],normal_path=pathList[3],image_path=pathList[2], mask_path=pathList[1], configPath=pathList[0], idx=frameIdx[1], args=args)
