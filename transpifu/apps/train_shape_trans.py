...
def train(opt, visualCheck_0=False):
    ...
    if opt.continue_train:
        ...
        opt.resume_name='PIFu_transformer'
        opt.resume_epoch=5
        opt.resume_iter=3199
    ...
    if opt.normal_activate: print("use normal!")
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        with tqdm(train_data_loader, unit="batch") as tepoch:
            ...
            for train_data in tepoch:  # 14496 iters for each epoch
                ...
                normal_tensor = None
                if opt.normal_activate: normal_tensor = train_data['normal'].to(device=cuda) # (B==2, num_views, C, W, H) RGB, 3x512x512 normal_images, float -1. ~ 1., bg is all ZEROS not -1.
                depth_tensor = None
                if opt.depth_activate: depth_tensor = train_data['depth'].to(device=cuda)  # (B==2, num_views, C, W, H) RGB, 3x512x512 normal_images, float -1. ~ 1., bg is all ZEROS not -1.
                ...
                if visualCheck_0:
                    ...
                    Image.fromarray(img_RGB).save("./sample_images/%s_img_input_by_PIL.png" % (
                        opt.name))  # PIL save RGB-array into proper-color.png

                # reshape tensors for multi-view settings
                image_tensor, _ = reshape_multiview_tensors(image_tensor, calib_tensor)  # (B * num_views, C, W, H)， (B * num_views, 4, 4)
                if depth_tensor!=None:
                    depth_tensor, _ = reshape_multiview_tensors(depth_tensor, calib_tensor)  # (B * num_views, C, W, H)， (B * num_views, 4, 4)
                normal_tensor, calib_tensor = reshape_multiview_tensors(normal_tensor,calib_tensor)  # (B * num_views, C, W, H)， (B * num_views, 4, 4)
                if opt.num_views > 1: sample_tensor = reshape_sample_tensor(sample_tensor, opt.num_views)  # (B * num_views, 3, n_in + n_out)

                # network forward pass
                res, error = netG.forward(image_tensor, sample_tensor, calib_tensor, labels=label_tensor, normal=normal_tensor, depth=depth_tensor)  # (B==2, 1, 5000), R
