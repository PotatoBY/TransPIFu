...
def gen_mesh_iccv(opt, net, cuda, data, save_path, use_octree=True):
    ...
    normal_tensor = None
    depth_tensor = None
    if opt.normal_activate: normal_tensor = data['normal'].to(device=cuda)  # (B==2, num_views, C, W, H) RGB, 3x512x512 normal_images, float -1. ~ 1., bg is all ZEROS not -1.
    if opt.depth_activate: depth_tensor = data['depth'].to(device=cuda)  # (B==2, num_views, C, W, H) RGB, 3x512x512 normal_images, float -1. ~ 1., bg is all ZEROS not -1.

    # use hour-glass networks to extract image features
    if normal_tensor!= None:
        net.filter(image_tensor,normal_tensor,depth_tensor)
    else:
        net.filter(image_tensor,depth_tensor)
...
def calc_error(opt, net, cuda, dataset, num_tests):
    ...
    with torch.no_grad():
        ...
        for idx in tqdm(range(num_tests)):
            ...
            # forward pass
            res, error = net.forward(image_tensor, sample_tensor, calib_tensor, labels=label_tensor, deepVoxels=deepVoxels_tensor, normal=normal_tensor, depth=depth_tensor) # (1, 1, n_in + n_out), R
