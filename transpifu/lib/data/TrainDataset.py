...
SENSEI_DATA_DIR          = "/mnt/data/Project/Geo-PIFu-master"
    ...
    def get_render_iccv(self, args, num_views, view_id_list, random_sample=False, dataConfig=None, view_id=None):
        ...
        for vid in view_ids:
            ...
            if True:
                ...
                # set paths
                depth_path = '%s/depthRGB/%06d.jpg' % (args.datasetDir, vid)
                if not os.path.exists(depth_path):
                    print("Can not find %s!!!" % (depth_path))
                    pdb.set_trace()
            ...
            if True:
                ...
                # set paths
                normal_path = '%s/normalRGB/%06d.jpg' % (args.datasetDir, vid)
            ...
            if True:
                ...
                # set path
                # load_npy = np.load(args.datasetDir + "/smplSemVoxels/105328.npy")
            ...
            if self.is_train and self.allow_aug:
                ...
                normal_padded = ImageOps.expand(normal_padded, pad_size, fill=0)
                depth_padded = ImageOps.expand(depth_padded, pad_size, fill=0)
                ...
                if self.opt.random_flip and np.random.rand() > 0.5:
                    ...
                    normal_padded = transforms.RandomHorizontalFlip(p=1.0)(normal_padded)
                    depth_padded = transforms.RandomHorizontalFlip(p=1.0)(depth_padded)
                ...
                if self.opt.random_scale:
                    ...
                    normal_padded = normal_padded.resize((w, h), Image.NEAREST)
                    depth_padded = depth_padded.resize((w, h), Image.NEAREST)
                ...
                normal_padded = normal_padded.crop((x1, y1, x1 + tw, y1 + th))
                depth_padded = depth_padded.crop((x1, y1, x1 + tw, y1 + th))

                # color space augmentation
                image_padded = self.aug_trans(image_padded)
                normal_padded = self.aug_trans(normal_padded)
                depth_padded = self.aug_trans(depth_padded)
            ...
            if True:
                ...
                normal_list.append(normal_padded)
            ...
            if True:
                ...
                depth_list.append(depth_padded)
