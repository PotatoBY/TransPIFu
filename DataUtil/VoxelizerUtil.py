...
import DataUtil.binvox_rw as binvox_rw
from subprocess import call
from scipy import ndimage


from DataUtil.TriBoxTest import tri_box_overlap
...
class SmplVtx(object):
    ...
    def __init__(self):
        ...
        # read the rest shape
        smpl_vtx_std = np.loadtxt('./SmplUtil/vertices.txt')
        ...
        # init. args.
        self.smpl_vtx_std_raw = smpl_vtx_std
        self.smpl_vtx_std = np.zeros(smpl_vtx_std.shape, np.float64)
        self.smpl_vtx_std_keepRatio = np.zeros(smpl_vtx_std.shape, np.float64)

        # each dimension is separately normalized, thus the D-H-W aspect ratio is broken
        self.smpl_vtx_std[:, 0] = (smpl_vtx_std[:, 0]-min_x)/(max_x-min_x) # [0,1]
        self.smpl_vtx_std[:, 1] = (smpl_vtx_std[:, 1]-min_y)/(max_y-min_y) # [0,1]
        self.smpl_vtx_std[:, 2] = (smpl_vtx_std[:, 2]-min_z)/(max_z-min_z) # [0,1]
        
        # normalize into [0,1] and keep the DHW ratio
        longestDim = np.max([max_x-min_x, max_y-min_y, max_z-min_z])
        self.smpl_vtx_std_keepRatio = smpl_vtx_std - np.array([[min_x, min_y, min_z]])
        self.smpl_vtx_std_keepRatio /= longestDim

_smpl_vtx = SmplVtx()

def get_smpl_std_vertex_raw():
    """
    the raw smpl rest shape vertices, without any normalization
    """
    
    return _smpl_vtx.smpl_vtx_std_raw

def get_smpl_std_vertex():
    """
    the original normalization with distortion
    """
    
    return _smpl_vtx.smpl_vtx_std

def get_smpl_std_vertex_keepRatio():
    """
    normalize into [0,1] and keep the DHW ratio
    """
    
    return _smpl_vtx.smpl_vtx_std_keepRatio
...
def calc_vmap_volume(smpl_volume, smpl_v, dim_h, dim_w, voxel_size):
    ...
    dim_h_half = dim_h/2 # 192/2
    dim_w_half = dim_w/2 # 128/2
    sigma = 0.05*0.05
    K = 4 # number of NN for KD-Tree searching

    smpl_std_v = get_smpl_std_vertex()

    x_dim, y_dim, z_dim = smpl_volume.shape[0], smpl_volume.shape[1], smpl_volume.shape[2] # 128, 192, 128
    smpl_v_volume = np.zeros((x_dim, y_dim, z_dim, 3), dtype=np.float32) # (128,192,128)

    kd_tree = scipy.spatial.KDTree(smpl_v) # create KD-Tree from 6890 SMPL vertices
    xx, yy, zz = np.where(smpl_volume>0) # xyz coords. of inside voxels
    oc_num = xx.shape[0] # N, number of inside voxels

    # convert xyz idx to metric coord., based on voxel_size 
    xx1 = np.expand_dims((xx - dim_w_half + 0.5) * voxel_size, axis=-1) # (N,1)
    yy1 = np.expand_dims((yy - dim_h_half + 0.5) * voxel_size, axis=-1) # (N,1)
    zz1 = np.expand_dims((zz - dim_w_half + 0.5) * voxel_size, axis=-1) # (N,1)

    pts = np.concatenate((xx1, yy1, zz1), axis=-1) # (N,3)
    dist_list, id_list = kd_tree.query(pts, k=K) # NN searching, need to verify that (the .obj is height normalized to 1.)
