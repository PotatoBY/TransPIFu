...
def load_ply_data(filename):
    ...
    for l in range(pc.shape[0]):
        ...
        # parse vertex cocordinate
        v_list.append((float(pc[l][0]), float(pc[l][1]), float(pc[l][2])))
        vc_list.append((float(pc[l][6])/255, float(pc[l][7])/255, float(pc[l][8])/255))
        ...
        # parse vertex normal
        vn_list.append((float(pc[l][3]), float(pc[l][4]), float(pc[l][5])))
        ...
        #
        # print(faces[l])
        # fv0, ft0, fn0 = segElementData(faces[l][0])
        # fv1, ft1, fn1 = segElementData(faces[l][1])
        # fv2, ft2, fn2 = segElementData(faces[l][2])
        f_list.append((faces[l][0][0], faces[l][0][1], faces[l][0][2]))
        # if ft0 is not None and ft1 is not None and ft2 is not None:
        #     ft_list.append((ft0, ft1, ft2))
        # if fn0 is not None and fn1 is not None and fn2 is not None:
        #     fn_list.append((fn0, fn1, fn2))
...
def save_obj_data_color(model, filename):
    assert 'v' in model and model['v'].size != 0
